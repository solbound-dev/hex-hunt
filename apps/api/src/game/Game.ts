...
export class Game {
    ...
    getAvailablePlayerPos(): Hex {
        if (this.grid.length === 0) {
            throw new Error(
                'Grid is empty, cannot get available player position.',
            );
        }
        let pos: Hex;
        while (true) {
    ...
    spawnCard() {
        const possibleHexes = this.grid.filter((h) => {
            ...
            const hexDisappeared = this.disappearedHexes.some((d) =>
                d.equals(h),
            );
    ...
    shootInDirection(directionHex: Hex, shooter: Player) {
        ...
        this.players.forEach((p) => {
            ...
            while (
                current.distanceTo(position) < RANGE &&
                this.isInGrid(position)
            ) {
        ...
        if (!shooter.lastBulletHex) {
            ...
            shooter.lastBulletHex = this.isInGrid(position)
                ? position
                : prevPosition;
        }
        return shotCard;
    ...
    checkCollisionAndUpdate(currentPlayer: Player) {
        ...
        this.players.forEach((p) => {
            ...
            if (p.isShooting) {
                ...
                otherPlayerNextPosition = new Hex(
                    p.pendingMove!.q,
                    p.pendingMove!.r,
                );
            }
