...
export class GameGateway implements OnGatewayConnection, OnGatewayDisconnect {
    ...
    async handleHostPrivateGame(
        ...
        if (!result) {
            console.log(
                'No available game found and failed to create a new one.',
            );
            return;
        }
    ...
    async handleQuickJoin(
        ...
        const result = await this.gameService.quickJoinGame(
            client.id,
            tier,
            token,
        );
        ...
        if (game.players.length === MAX_PLAYERS) {
            console.log(
                'fourth player joined, game supposed to be initialized',
            );

            const game = await this.gameService.startGame(gameId, this.server);
            if (!game) {
                console.log(
                    `Game ${gameId} does not exist or is already started`,
                );
                return;
            }
            this.server.to(gameId).emit('gameStart', game.serialize());
